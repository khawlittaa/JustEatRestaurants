...
extension Array where Element == Restaurant {
    ...
    func reordebyStatus() -> [Restaurant] {
        ...
        return self.sorted { (a, b) -> Bool in
            if let first = defaultOrder.firstIndex(of: a.status), let second = defaultOrder.firstIndex(of: b.status) {
                return first < second
            }
            return false
        }
    ...
    func reorderRestaurants() -> [Restaurant] {
        ...
        return self.sorted { (res1taurant1, restaurant2) -> Bool in
            ...
            if isfavorite(res1taurant1) == isfavorite(restaurant2){
                if let first = defaultOrder.firstIndex(of: res1taurant1.status), let second = defaultOrder.firstIndex(of: restaurant2.status) {
                    return first < second
                }
                return false
            }else{
